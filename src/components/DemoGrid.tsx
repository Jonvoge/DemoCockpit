...
export function DemoGrid({ pinnedDemos, otherDemos, currentUser, onEdit, onPin, onCopy, onInfo, onClick }: Props) {
  return (
    <div className="pb-6">
      {pinnedDemos.length > 0 && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 px-4">
            {pinnedDemos.map(d => (
              <DemoCard key={d.id} demo={d} isPinned={true} currentUser={currentUser} onEdit={onEdit} onPin={onPin} onCopy={onCopy} onInfo={onInfo} onClick={onClick} />
            ))}
          </div>
        </div>
      )}
      <div>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 px-4">
          {otherDemos.map(d => (
            <DemoCard key={d.id} demo={d} isPinned={false} currentUser={currentUser} onEdit={onEdit} onPin={onPin} onCopy={onCopy} onInfo={onInfo} onClick={onClick} />
          ))}
        </div>
